# -*- coding: utf-8 -*-
# utils by wukesonguo
# notebook: notebook-1-sw-division
# indicator group by sw industry
# use load_detail_indicator()
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    for _lev in [1, 2]:
        ...
        for _sw_lev_name in stock_sw_level_name["sw_lev{}".format(_lev)]:
            ...
            print(save_lev_name)
            stock_sw_level_dict = {"sw_lev1": [], "sw_lev2": [], "sw_lev3": []}
            for idx, current_date in enumerate(N_XSHG_DATES):
                ...
                calcu_stock_sw_indicator(stock_newest, _lev, stock_sw_level_dict, current_date)
            _sw_lev_item = pd.concat(stock_sw_level_dict["sw_lev{}".format(_lev)])
            ...
            _sw_lev_item.to_csv('stock_sw_1_daily/{}'.format(save_lev_name), encoding='utf_8_sig')
            ...
            plt.show()
            plt.title("PE_weight")
            _sw_lev_item.PE_weight.ewm(span=5).mean().plot()
            plt.show()
            plt.title("ROE")
            _sw_lev_item.ROE.ewm(span=5).mean().plot()
            plt.show()
            plt.title("ROE_weight")
            _sw_lev_item.ROE_weight.ewm(span=5).mean().plot()
            plt.show()
