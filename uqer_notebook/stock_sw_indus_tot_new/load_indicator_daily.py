...
def load_sw_industry(stock_base):
    ...
    t_stock_base = t_stock_base[t_stock_base["isNew"] == 1]
    t_stock_base = t_stock_base.drop(["isNew"], axis=1)
    # print(stock_base.head())
...
def stock_value_investment(stock_base):
    df_factor = copy.deepcopy(stock_base)
    # Strategy: 柏顿.墨基尔投资理念
    ...
    # # 净利润增长率处于市场最高的20%。
    # # 较长的调仓频率。
    secShortName_list = df_factor[(df_factor['PE'] < df_factor['PE'].quantile(0.2))
                            &(df_factor['NetProfitGrowRate'] > df_factor['NetProfitGrowRate'].quantile(0.8))].secShortName.tolist()
    df_factor["value_factor_1"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)
    ...
    secShortName_list = df_factor[df_factor['masterValue'] >= 0].secShortName.tolist()
    df_factor["value_factor_2"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)

    # Strategy: 罗伯特•山朋投资理念
    # # 市销率大于0且低于市场正值平均值
    # # 市现率大于0且低于市场正值平均值
    # # 管理层持股比例大于市场均值（无数据此处未加入）
    secShortName_list = df_factor[(df_factor['PS'] > 0)
                    &(df_factor['PCF'] > 0)
                    &(df_factor['PS'] < df_factor['PS'].mean())
                    &(df_factor['PCF'] < df_factor['PCF'].mean())].secShortName.tolist()
    df_factor["value_factor_3"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)
    ...
    secShortName_list = df_factor[(df_factor['PB'] < 2)
                    &(df_factor['DebtsAssetRatio'] < df_factor['DebtsAssetRatio'].mean())].secShortName.tolist()
    df_factor["value_factor_4"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)
    ...
    # # 5年权益回报率大于市场平均值
    # # 5年资产回报率大于市场平均值
    secShortName_list = df_factor[(df_factor['PCF'] > 0)  # addition
                        &(df_factor['PB'] > 0)  # addition
                        &(df_factor['PE'] > 0)  # addition
                        &(df_factor['PS'] > 0)  # addition
                        &(df_factor['PB'] < df_factor['PB'].mean())
                        ...
                        &(df_factor['CTOP'] > df_factor['CTOP'].mean())].secShortName.tolist()
    df_factor["value_factor_5"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)
    ...
    # # 股票的市净率小于2.0
    # # 股票最近一季负债净值比小于80%
    secShortName_list = df_factor[(df_factor['PB'] > 0)  # addition
                        &(df_factor['PE'] > 0)  # addition
                        &(df_factor['PCF'] > 0)  # addition
                        &(df_factor['PE'] < 1.0 * df_factor['PE'].mean())
                        &(df_factor['PS'] < 1.0 * df_factor['PS'].mean())
                        &(df_factor['PCF'] < 1.0 * df_factor['PCF'].mean())
                        &(df_factor['PB'] < 2)
                        &(df_factor['DebtEquityRatio'] < 0.8)].secShortName.tolist()
    df_factor["value_factor_6"] = df_factor["secShortName"].apply(lambda x: 1 if x in secShortName_list else 0)

    return df_factor

def calcu_stock_sw_indicator(stock_newest, lev, stock_sw_level_list, current_date):
    _lev = lev
    _lev_names_joint = "sw_lev{}_name_joint".format(_lev)

    if _lev == 4:  # used for stock market
        stock_newest[_lev_names_joint] = stock_newest["tickerPrefixH2"]
    if _lev == 5:
        stock_newest[_lev_names_joint] = "Chinese"

    feature_rank = 'tradeMarketValue'
    stock_newest['sw_lev{}_tmv_rank_asc'.format(_lev)] = \
        stock_newest.groupby(_lev_names_joint)[feature_rank].rank(ascending=True)
    stock_newest['sw_lev{}_tmv_rank_dsc'.format(_lev)] = \
        stock_newest.groupby(_lev_names_joint)[feature_rank].rank(ascending=False)
    if _lev == 1:
    ...
    _tot_marketValue = stock_newest.groupby(_lev_names_joint, as_index=False)["marketValue"].agg(
        {"totSWMarketValue": np.sum})
    _tot_tradeMarketValue = stock_newest.groupby(_lev_names_joint, as_index=False)["tradeMarketValue"].agg(
        {"totSWTradeMarketValue": np.sum})
    _tot_turnoverValue = stock_newest.groupby(_lev_names_joint, as_index=False)["turnoverValue"].agg(
        {"totSWTurnoverValue": np.sum})
    stock_newest = pd.merge(stock_newest, _tot_marketValue, how='left', on=_lev_names_joint)
    stock_newest = pd.merge(stock_newest, _tot_tradeMarketValue, how='left', on=_lev_names_joint)
    stock_newest = pd.merge(stock_newest, _tot_turnoverValue, how='left', on=_lev_names_joint)
    stock_newest["marketValueProportion"] = stock_newest["marketValue"] / stock_newest["totSWMarketValue"]
    stock_newest["tradeMarketValueProportion"] = stock_newest["tradeMarketValue"] / stock_newest["totSWTradeMarketValue"]
    stock_newest["turnoverValueProportion"] = stock_newest["turnoverValue"] / stock_newest["totSWTurnoverValue"]
    # print(stock_newest["totMarketValueProportion"].sum())
    ...
    # print(stock_newest["turnoverValueProportion"].sum())
    stock_newest["turnoverRateWeight"] = stock_newest["turnoverRate"] * stock_newest["tradeMarketValueProportion"]
    stock_newest["close_price_rise_rate_weight"] = \
        stock_newest["close_price_rise_rate"] * stock_newest["tradeMarketValueProportion"]
    stock_newest["PEWeight"] = stock_newest["PE"] * stock_newest["tradeMarketValueProportion"]

    # group by(_lev_names_joint)
    stock_newest["stockCnt"] = stock_newest["closePrice"]
    _df_sw_lev_cnt = stock_newest.groupby(_lev_names_joint)["stockCnt"].count()
    ...
    _df_sw_lev_PEWeight = stock_newest.groupby(_lev_names_joint)['PEWeight'].sum()

    _df_sw_lev_market_value_prop = stock_newest.groupby(_lev_names_joint)['totTradeMarketValueProportion'].sum()
    _df_sw_lev_turnover_value_prop = stock_newest.groupby(_lev_names_joint)['totTurnoverValueProportion'].sum()
    _df_sw_lev_turnover2market_rate = _df_sw_lev_turnover_value_prop / _df_sw_lev_market_value_prop
    ...
    _df_sw_lev_allinone = pd.DataFrame([_df_sw_lev_cnt, _df_sw_lev_trade_turnover_value,
                                        _df_sw_lev_PE, _df_sw_lev_PEWeight,
                                        _df_sw_lev_riseup_rate, _df_sw_lev_trade_turnover_rate,
                                        _df_sw_lev_riseup_rate_weight, _df_sw_lev_trade_turnover_rate_weight,
                                        _df_sw_lev_turnover_value_prop, _df_sw_lev_market_value_prop,
                                        _df_sw_lev_turnover2market_rate]).T
    _df_sw_lev_allinone["tradeDate"] = current_date
    _df_sw_lev_allinone.rename(columns={"Unnamed 0": "turnover2market_rate"}, inplace=True)
    stock_sw_level_list.append(copy.deepcopy(_df_sw_lev_allinone))
