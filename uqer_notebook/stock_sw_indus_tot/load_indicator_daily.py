...
def load_newest_stock(n_current_date):
    ...
    stock_base["int_listDateDelta"] = stock_base["listDateDelta"].apply(lambda x: int(x))
    return stock_base
...
def stock_value_investment(stock_base):
    ...
    # # 净利润增长率处于市场最高的20%。
    # # 较长的调仓频率。
    secShortName_list = df_factor[(df_factor['PE'] < df_factor['PE'].quantile(0.25))
                            &(df_factor['NetProfitGrowRate'] > df_factor['NetProfitGrowRate'].quantile(0.75))].secShortName
    ...
    secShortName_list = df_factor[df_factor['masterValue'] >= 0].secShortName
    ...
    secShortName_list = df_factor[df_factor['PS'] > 0
                    ...
                    &(df_factor['PCF'] < df_factor['PCF'].mean())].secShortName
    ...
    secShortName_list = df_factor[df_factor['PB'] < 2
                    &df_factor['DebtsAssetRatio'] < df_factor['DebtsAssetRatio'].mean()].secShortName
    ...
    secShortName_list = df_factor[df_factor['PCF'] > 0  # addition
                        ...
                        &(df_factor['CTOP'] > df_factor['CTOP'].mean())].secShortName
    ...
    df_factor = df_factor[df_factor['PB'] > 0  # addition
                        ...
                        & (df_factor['PB'] < 2)
                        & (df_factor['DebtEquityRatio'] < 0.8)].secShortName
...
def calcu_stock_sw_indicator(stock_newest, lev, stock_sw_level_list, current_date):
    ...
    tot_turnoverValue = stock_newest.turnoverValue.sum()
    ...
    stock_newest["turnoverValueProportion"] = stock_newest["turnoverValue"] / tot_turnoverValue
