...
def load_newest_stock():
    ...
    stock_base = DataAPI.EquGet(secID=u"", ticker=u"", equTypeCD=u"A", listStatusCD=u"L", exchangeCD="",
                                ListSectorCD=u"", field=_field, pandas="1")
...
def stock_value_investment(_df_factor, stock_sector='ALL', sw_industry='ALL'):
    ...
    # df_factor = df_factor[df_factor['DilutedEPS'] >= 0.01]  # addition
    # df_factor = df_factor[df_factor['EGRO'] >= 0.00]  # addition
    # df_factor['tradeShares'] = df_factor['tradeMarketValue'] / df_factor['closePrice']
    # df_factor['masterValue'] = df_factor['tradeShares'] * df_factor['DilutedEPS'] * (8.0 + 4 * df_factor['EGRO'])
