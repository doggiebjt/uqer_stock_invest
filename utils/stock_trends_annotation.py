# -*- coding: utf-8 -*-
# stock medium term trends

import numpy as np
import pandas as pd

stock_trends_annotation = {
    "000016":
        [
...
stock_trends_annotation_t = dict()
for key, val in stock_trends_annotation.items():
    _len = len(val)
    stock_trends_annotation_t[key] = (val[0][0], val[_len - 1][1])
print(stock_trends_annotation_t.keys())

stock_trends_annotation_dict = dict()
for key, val in stock_trends_annotation.items():
    _ticker = key
    for idx, item in enumerate(val):
        _start_day, _end_day, _label = item
        if _ticker not in stock_trends_annotation_dict:
            stock_trends_annotation_dict[_ticker] = []
        _start_day = int(_start_day.replace("-", ""))
        _end_day = int(_end_day.replace("-", ""))
        _label = int(_label)
        stock_trends_annotation_dict[_ticker].append((range(_start_day, _end_day + 1), _label))
print(stock_trends_annotation_dict.keys())


def stock_trends_annotation_addition(stock_df, label_name, stock_trends_annotation_dict):
    label_list = []
    stock_df = stock_df.insert(stock_df.shape[1], label_name, 0)
    for _idx, row in stock_df.iterrows():  # row -> deepcopy
        ticker = row["ticker"]
        date = row["n_date"]
        for _scope, _label in stock_trends_annotation_dict[ticker]:
            if date in _scope:
                label_list.append(_label)
                # stock_df[label_name][_idx] = _label
                continue
    stock_df[label_name] = pd.Series(label_list)
    return stock_df
